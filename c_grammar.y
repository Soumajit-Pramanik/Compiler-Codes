...
int str_type_no=50, first_type=0;
...
%type <ex> logical_and_expression conditional_expression assignment_expression expression constant_expression
%type <blockno> '{'
...
postfix_expression
	...
	| postfix_expression '[' expression ']'                                    {if(un_decl_flag==0){ $$.exp.etype=$1.exp.etype;
                                                                                   if($3.exp.etype%100==15||$3.exp.etype%100==16) 
                                                                                          printf("\nLine no %d--> Error:Array index must be integral\n",lineno);
                                                                                   else if($3.exp.etype%100==11)
	...
	| postfix_expression '(' ')'                                               {if(un_decl_flag==0){ $$.exp.etype=($1.symb)->sp_type;
                                                                                   if($1.symb->no_arg!=_last_checked_index)
                                                                                          printf("\nLine no %d--> Error: No of arguments mismatch index=%d\n",lineno,_last_checked_index);
...
declaration_specifiers
	...
	| storage_class_specifier declaration_specifiers           {$$=$2*100+$1;}
	| type_specifier                                           {$$=$1;}  
	| type_specifier declaration_specifiers                    {$$=$2*100+$1;}
	| type_qualifier                                           {$$=$1;}
	| type_qualifier declaration_specifiers                    {$$=$2*100+$1;}
	;
...
direct_declarator
	: IDENTIFIER                                        {$$=$1;}
	| '(' declarator ')'                                {}  
	| direct_declarator '[' constant_expression ']'     {if($3.exp.etype%100==15||$3.exp.etype%100==16) 
                                                                  printf("\nLine no %d--> Error:Array index must be integral\n",lineno);
                                                            else if($3.exp.etype%100==11)
                                                                  printf("\nLine no %d--> Warning:Array index is char\n",lineno);}
	| direct_declarator '[' ']'
	| direct_declarator M parameter_type_list ')'     {    
								$1->sp_type=$<dtype>0;$1->token=2;
							             if(DEBUG)printf("\nLine no %d--> function: name %s,ret type %d, no  of args=%d Parameter types are:",lineno,$1->lexeme,$1->sp_type,$1->no_arg);
                                                            ...
                                                            while(p!=NULL)
                                                                    {if(DEBUG)printf("%d\t",lineno,p->arg_type);p=p->next;}
							    if(_last_checked_index!=-1&&$1->no_arg!=_last_checked_index)
                                                                    printf("\nLine no %d--> Error: No of arguments mismatch index =%d",lineno,_last_checked_index);
                                                            _last_checked_index = -1; _right_paren_func=1;}
	| direct_declarator '(' identifier_list ')'         {} 
	| direct_declarator '(' ')'                         {$1->sp_type=$<dtype>0;$1->token=2;
...
direct_abstract_declarator
	...
	| '[' constant_expression ']'
	| direct_abstract_declarator '[' ']'
	| direct_abstract_declarator '[' constant_expression ']'   {if($3.exp.etype%100==15||$3.exp.etype%100==16) 
                                                                           printf("\nLine no %d--> Error:Array index must be integral\n",lineno);
                                                                   else if($3.exp.etype%100==11)
                                                                           printf("\nLine no %d--> Warning:Array index is char\n",lineno);}
	| '(' ')'
...
function_definition
	...
	| declaration_specifiers declarator compound_statement         {if($2->sp_type==0)
									         ...
									         $2->sp_type=$1; $2->token=2;
                                                                             ...
                                                                             if($2->sp_type!=$1)
										{
											if(DEBUG)printf("\nLine no %d--> Function return type mismatch\n",lineno);
										}
...
int type_error(int type1,int type2)
{
    if ((type1%100)<(type2%100))
      {printf("\nLine no %d-->Warning: Type conversion may change the value of RHS\n",lineno);return 0;}
    else
      return (type1);
}
